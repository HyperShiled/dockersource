...
import (
	...
	"github.com/compassorg/postgresql-service-broker/models"
...
func readBrokerSettings() *brokerapi.Catalog {
	section, _ := beego.AppConfig.GetSection("postgresql-service-broker")
...
func generatePostgreSQLUri(username, password, address, databaseName string) string {
	// "postgres://postgres:12345678@192.168.21.87:5432?sslmode=disable"
	return fmt.Sprintf("postgres://%s:%s@%s/%s?sslmode=disable", username, password, address, databaseName)
}
