...
import (
	...
	"github.com/compassorg/mysql-service-broker/models"
...
func readBrokerSettings() *brokerapi.Catalog {
	section, _ := beego.AppConfig.GetSection("mysql-service-broker")
...
func generateMySQLUri(username, password, address, databaseName string) string {
	return fmt.Sprintf("%s:%s@tcp(%s)/%s", username, password, address, databaseName)
}
