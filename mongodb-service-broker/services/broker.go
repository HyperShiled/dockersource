...
var (
	mongodbServiceBroker *MongoDBServiceBroker
	lastOperationMap    map[string]int64
)

type MongoDBServiceBroker struct {
}

func MongoDBServiceBrokerInstance() *MongoDBServiceBroker {
	if mongodbServiceBroker == nil {
		mongodbServiceBroker = &MongoDBServiceBroker{}
	}
	return mongodbServiceBroker
}

func (o *MongoDBServiceBroker) Catalog() *brokerapi.Catalog {
	return readBrokerSettings()
}

func (o *MongoDBServiceBroker) ServiceInstance(id string) (userProvidedServiceInstance, error) {
...
func (o *MongoDBServiceBroker) Provision(id string, req *brokerapi.CreateServiceInstanceRequest) (*brokerapi.CreateServiceInstanceResponse, error) {
	...
	if !ok {
		...
		usernamePointer, passwordPointer, urlPointer, err := getMongoConnectionInfo(connectURI.(string))
		if err != nil {
			return nil, err
		}
		logs.Info("username: ", *usernamePointer)
		logs.Info("password: ", *passwordPointer)
		logs.Info("url: ", *urlPointer)
		databaseName, userName, userPassword, err := createDatabaseAndUser(*urlPointer, *usernamePointer, *passwordPointer, id, planValue)
		if err != nil {
			logs.Error(err)
			return nil, errors.New("CRUD - Create database and user error.")
		}
		logs.Info("databaseName: ", *databaseName)
		logs.Info("userName: ", *userName)
		logs.Info("userPassword: ", *userPassword)
		DashboardURL = generateMongoDBUri(*userName, *userPassword, connectURI.(string), *databaseName)
...
func (o *MongoDBServiceBroker) DeProvision(id string) (*brokerapi.DeleteServiceInstanceResponse, error) {
	//o.rwMutex.Lock()
	...
	var serviceInstance userProvidedServiceInstance
	err = json.Unmarshal([]byte(response.Node.Value), &serviceInstance)
	if err != nil {
		logs.Error(err)
		return nil, err
	}
	...
	if ok {
		...
		logs.Info("begin")
		logs.Info("connectURI:", connectURI)
		logs.Info("databaseName:", databaseName)
		logs.Info("userName:", userName)
		usernamePointer, passwordPointer, urlPointer, err := getMongoConnectionInfo(connectURI.(string))
		if err != nil {
		...
		err = deleteDatabaseAndUser(*urlPointer, *usernamePointer, *passwordPointer, databaseName.(string), userName.(string))
		if err != nil {
			logs.Error(err)
...
func (o *MongoDBServiceBroker) Binding(instanceID, bindingID string, req *brokerapi.BindingRequest) (*brokerapi.CreateServiceBindingResponse, error) {
	//o.rwMutex.RLock()
...
func (o *MongoDBServiceBroker) UnBinding(instanceId, bindingId string) error {
	// Since we don't persist the binding, there's nothing to do here.
	return nil
}

func (o *MongoDBServiceBroker) ServiceInstanceLastOperation(instanceID, serviceID, planID, operation string) (*brokerapi.LastOperationResponse, error) {
