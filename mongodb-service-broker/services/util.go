...
import (
	...
	"github.com/compassorg/mongodb-service-broker/models"
...
func readBrokerSettings() *brokerapi.Catalog {
	section, _ := beego.AppConfig.GetSection("mongodb-service-broker")
...
func getPlanValue(plan *brokerapi.ServicePlan) string {
	...
	if len(metadata.Costs) > 0 {
		...
		valueUnit = cost.Amount.Value + cost.Unit
	}

	return valueUnit
}

func generateMongoDBUri(username, password, address, databaseName string) string {
	// myUserAdmin:abc123@192.168.21.87:27017/admin
	return fmt.Sprintf("%s:%s@%s/%s", username, password, address, databaseName)
}
